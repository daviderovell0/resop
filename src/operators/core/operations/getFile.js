...
opn.defineOptions({
  ...
  destination:
    'full path to local folder/file',
});
...
function exec() {
  ...
  if(!retobj.success) { // append filename
    opn.error(`${opn.options.source} not a file`)
  }

  // if dest is an existing local dir, appenf source filename
  if (fs.existsSync(opn.options.destination)) {
    let idx = opn.options.source.lastIndexOf('/') +1;
    if (idx === -1 ) idx = 0;
    const filename = opn.options.source.slice(
      idx,
  ...
  retobj = sshell.scpRecv(opn.options.source, opn.options.destination, opn.user);
